...
/// <summary>Provides ValueTask-based asynchronous bind operations that aggregate all errors for <see cref="Result{T}"/> types.</summary>
public static partial class ValueResultExtensions
{
    public static async ValueTask<Result<TValue>> BindAllAsync<TValue>(this Result<TValue> result, ValueTask<Result> next)
    {
    ...
    public static async ValueTask<Result<TValue>> BindAllAsync<TValue>(this ValueTask<Result<TValue>> result, Result next)
        => (await result.ConfigureAwait(false)).BindAll(next);

    public static async ValueTask<Result<TValue>> BindAllAsync<TValue>(this ValueTask<Result<TValue>> result, ValueTask<Result> next)
