...
{
    [Generator]
    public class TaskEnsureExtensionsGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            context.RegisterPostInitializationOutput(ctx =>
            {
                var fileName = "ValueResult.Task.EnsureExtensions.g.cs";
                ...
                for (int i = 2; i <= MaxElements; i++)
                {
                    ...
                    builder.Append($$"""
                                     public static async Task<Result<({{types}})>> EnsureAsync<{{types}}>(
