...
namespace FluentUnions.Generators.Results
{
    ...
    public class TaskBindExtensionsGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            {
                var fileName = "ValueResult.Task.BindExtensions.g.cs";
                ...
                for (int i = 2; i <= MaxElements; i++)
                {
                    ...
                    builder.Append($$"""
                                         public static Task<Result<TTarget>> BindAsync<{{types}}, TTarget>(
                                             in this Result<({{types}})> source,
                                             Func<{{types}}, Task<Result<TTarget>>> binder)
                                         {
                                         ...
                                         public static async Task<Result<TTarget>> BindAsync<{{types}}, TTarget>(
