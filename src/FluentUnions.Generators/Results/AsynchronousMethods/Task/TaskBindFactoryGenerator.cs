...
{
    [Generator]
    public class TaskBindFactoryGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            context.RegisterPostInitializationOutput(ctx =>
            {
                var className = "ValueResult.Task.BindFactory";
                ...
                for (int i = 2; i <= MaxElements; i++)
                {
                    ...
                    builder.Append(Tab(1) + $"public static async Task<Result<({types})>> BindAsync<{types}>(\n");
                    builder.Append(string.Join(",\n",
                        Enumerable.Range(1, i).Select(n => $"{Tab(2)}Func<Task<Result<TValue{n}>>> result{n}")));
                    builder.Append($")\n{Tab(1)}{{\n");
