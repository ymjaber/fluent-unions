...
{
    [Generator]
    public class ValueTaskBindAllAppendExtensionsGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            context.RegisterPostInitializationOutput(ctx =>
            {
                var fileName = "ValueResult.ValueTask.BindAllAppendExtensions.g.cs";
                ...
                for (int i = 2; i <= MaxElements; i++)
                {
                    for (int j = 1; j < i; j++)
                    {
                        ...
                        builder.Append($$"""
                                             public static async ValueTask<Result<({{sourceTypes}}, {{targetTypes}})>> BindAllAppendAsync<{{sourceTypes}}, {{targetTypes}}>(
                                                 this Result<{{tupleSourceTypes}}> result,
                                                 ValueTask<Result<{{tupleTargetTypes}}>> binder)
                                             {
